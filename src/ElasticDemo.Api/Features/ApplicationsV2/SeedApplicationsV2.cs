...
public record SeedApplicationsV2Response(
    bool Success,
    string Message,
    int TotalProcessed,
    int SuccessCount,
...
public class SeedApplicationsV2Handler(
    ElasticsearchClient client,
    ILogger<SeedApplicationsV2Handler> logger)
{
    ...
    private async Task<BatchResult> SendBatchAsync(List<ApplicationV2> batch, int batchNumber)
    {
        try
        {
            var bulkResponse = await client.BulkAsync(b => b
                .Index(ApplicationV2Index.Alias)
                .IndexMany(batch)
            );
    ...
    {
        logger.LogInformation("Seeding applications V2 with BatchSize={BatchSize}", BatchSize);

        var currentBatch = new List<ApplicationV2>(BatchSize);
        ...
        try
        {
            await foreach (var application in JsonSerializer.DeserializeAsyncEnumerable<ApplicationV2>(body, JsonOptions))
            {
        ...
        return Results.Ok(new SeedApplicationsV2Response(
            success,
            message,
            totalProcessed,
            successCount,
